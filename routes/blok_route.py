...
from model import Blok, Lahan, DataBrix

blok_bp = Blueprint("blok", __name__)

@blok_bp.route("/blok", methods=["POST"])
def create_blok():
...
@blok_bp.route("/blok", methods=["GET"])
def get_all_blok():
...
@blok_bp.route("/blok/<int:id>", methods=["GET"])
...
@blok_bp.route("/blok/<int:id>", methods=["PUT"])
def update_blok(id):
    blok = Blok.query.get(id)
    if not blok:
        return jsonify({"message": "Blok tidak ditemukan"}), 404
...
@blok_bp.route("/blok/<int:id>", methods=["DELETE"])
def delete_blok(id):
    blok = Blok.query.get(id)
    if not blok:
        return jsonify({"message": "Blok tidak ditemukan"}), 404
